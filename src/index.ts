 ...
 *
 * Community edition — connects directly to Google Docs API v1.
 *
 * Usage (stdio - for Claude Desktop / Cursor / VS Code):
 *   GOOGLE_CLIENT_ID=xxx GOOGLE_CLIENT_SECRET=xxx GOOGLE_REFRESH_TOKEN=xxx npx @node2flow/google-docs-mcp
 *
...
async function startHttp() {
  ...
  const config = getConfig();
  app.listen(port, () => {
    console.log(`Google Docs MCP Server (HTTP) listening on port ${port}`);
